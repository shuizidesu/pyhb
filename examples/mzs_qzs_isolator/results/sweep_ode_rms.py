...
try:  # package import
    from .compare_time_history import INTEGRATION_METHODS, make_mzs_qzs_rhs, reconstruct_hb_time_history
except ImportError:  # direct script execution
    from compare_time_history import INTEGRATION_METHODS, make_mzs_qzs_rhs, reconstruct_hb_time_history


DEFAULT_OUTPUT_FIG = Path(__file__).resolve().parent / "ode_sweep_rms.png"
DEFAULT_DATA_OUTPUT = Path(__file__).resolve().parent / "ode_sweep_rms.npz"
DEFAULT_HB_INPUT = Path(__file__).resolve().parent / "autodiff_arc.npz"
...
def validate_sweep_settings(
    ...
    if history_periods <= 0:
        raise ValueError("history_periods must be positive")
...
def integrate_one_speed(
    ...
    return theta, state[:, : model.n_dof], state[:, model.n_dof :]


def tail_time_history(
    ...
    tail_count = int(history_periods * points_per_period) + 1
    tail_count = min(tail_count, int(theta.size))
    history = np.empty((tail_count, 3), dtype=np.float64)
    history[:, 0] = theta[-tail_count:]
    history[:, 1] = displacement[-tail_count:, 0]
    history[:, 2] = phase_velocity[-tail_count:, 0]
    return history
...
def run_sweep(
    ...
    validate_sweep_settings(settle_periods, rms_periods, history_periods, points_per_period)
    ...
    tail_count = int(history_periods * points_per_period) + 1
    time_history = np.empty((omega_values.size, tail_count, 3), dtype=np.float64)
    ...
    for index, omega in enumerate(omega_values, start=1):
        ...
        theta, displacement, phase_velocity = integrate_one_speed(
            model,
            float(omega),
            current_state,
            total_periods,
        ...
        current_state = np.concatenate((displacement[-1], phase_velocity[-1]))
        final_state_history[index - 1] = current_state
        success_flag[index - 1] = True
        time_history[index - 1] = tail_time_history(
            theta,
            displacement,
            phase_velocity,
            history_periods,
    ...
    return rms_history, final_state_history, success_flag, time_history
...
def compute_hb_rms(
    ...
    with np.load(input_path) as data:
        ...
        coefficient_history = np.asarray(data["coefficient_history"], dtype=np.float64)
    period = 2.0 * np.pi / FREQUENCY_RESOLUTION
    theta = np.arange(points_per_period, dtype=np.float64) * (period / points_per_period)
    hb_item, _, _ = generate_hb_items(theta, HARMONICS)
...
def save_data(
    ...
    np.savez(
        ...
        history_periods=np.asarray(history_periods, dtype=np.int64),
        points_per_period=np.asarray(points_per_period, dtype=np.int64),
        frequency_resolution=np.asarray(FREQUENCY_RESOLUTION, dtype=np.float64),
...
def run_from_args(args: argparse.Namespace) -> None:
    ...
    rms_history, final_state_history, success_flag, time_history = run_sweep(
        model,
        omega_values,
        dofs,
        args.settle_periods,
        args.rms_periods,
        args.history_periods,
    ...
    if args.hb_input is not None:
        if Path(args.hb_input).exists():
            hb_omega, hb_rms = compute_hb_rms(args.hb_input, dofs, args.points_per_period)
        else:
            print(f"HB input {args.hb_input} does not exist; skipping HB RMS overlay")
