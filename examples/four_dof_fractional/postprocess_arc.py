...
from examples.four_dof_fractional.model import default_four_dof_system, FourDofFractionalModel
from examples.four_dof_fractional.run_arc import DEFAULT_PLOT_DOFS, FRACTIONAL_HARMONICS, FREQUENCY_RESOLUTION
...
def compute_stability_history(
    ...
    model = FourDofFractionalModel(default_four_dof_system())
    ...
    for parameter, coefficients in zip(parameter_history, coefficient_history, strict=True):
        result = compute_floquet(model, coefficients, float(parameter), FRACTIONAL_HARMONICS, FREQUENCY_RESOLUTION, config)
