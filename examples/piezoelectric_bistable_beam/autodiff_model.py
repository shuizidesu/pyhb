...
class PiezoelectricBistableBeamParameters:
    a: float = 56e-3
    b: float = 110e-3
    mass: float = 12.34e-3
    damping: float = 0.01 * 12.34e-3
    k0: float = 2.3294e9
    k1: float = 2.1243e3
    k2: float = -0.0229
    k3: float = 1.9033e-4
    capacitance: float = 1.6427e-8
    resistance: float = 500e3


class PiezoelectricBistableBeamAutodiffModel(AutodiffSecondOrderTimeModel):
    """Bistable beam with piezoelectric coupling from references/beam_piezoelectric.md."""

    def __init__(self, parameters: PiezoelectricBistableBeamParameters | None = None) -> None:
        self.parameters = parameters or PiezoelectricBistableBeamParameters()
        p = self.parameters
        self.mass = np.array(
            [
                [p.mass, 0.0],
    ...
    def local_nonlinear_force_torch(
        ...
        voltage = local_x[:, 1]
        q_tau = local_dx[:, 0]
        q_dot = parameter * q_tau
        p = self.parameters

        geometry = torch.sqrt(float(p.a) ** 2 - q.pow(2))
        lever = 0.5 * float(p.b) - geometry
        mechanical_force = (
            4.0 * float(p.k0) * lever.pow(3) * q / geometry
            + 2.0 * (float(p.k1) + 2.0 * float(p.k2) * voltage) * lever * q / geometry
            - 2.0 * float(p.k3) * voltage * q / geometry
        )
        electrical_force = 2.0 * float(p.k3) * q_dot * q / geometry - 4.0 * float(p.k2) * q_dot * lever * q / geometry
        return torch.stack((mechanical_force, electrical_force), dim=1)
